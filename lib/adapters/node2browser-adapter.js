...
function Node2BrowserAdapter(urls, oCrawler, callback, interval_, timeout_) {
  ...
  function run() {
    ...
    if (typeof fn.domCallback == 'function')
      msg.domCallback = fn.domCallback.toString()
...
function init() {
  ...
  server.listen(9998, function() {
    ...
    browser.onerror = function() {
      console.log('\nNode2browser just supports chrome for now,' +
      'but you could open another browser installed with tentacle.user.js' +
      ' to visit "http://localhost:9998" to continue, if you see this.\n')
    }
